...
// Een project met een echt app-icoon in de map is daaraan beter te herkennen
// dan aan een emoji: het is hetzelfde plaatje dat op je telefoon staat. Main
// zoekt het op en filtert het Flutter-sjabloonicoon eruit — zonder dat filter
// zou elk vers project hetzelfde blauwe logo krijgen en was de zijbalk juist
// onleesbaarder geworden.
//
// De uitkomst bewaren we per pad. De zijbalk hertekent bij elke klik en mag
// daar geen bestanden voor laten inlezen.
const projIcoonPerPad = new Map()   // pad -> { ok, dataUrl, soort, reden }
...
async function ververesProjIcoonPad(pad) {
  ...
  try {
    ...
    return projIcoonPerPad.get(pad)
  } catch {
    projIcoonPerPad.set(pad, { ok: false, reden: 'fout' })
    return null
  } finally {
...
// zijn cache, wij de onze, en de eerstvolgende tekening haalt het weer op.
async function vergeetProjIcoon(pad) {
  if (pad) projIcoonPerPad.delete(pad)
  else projIcoonPerPad.clear()
  try { await window.api.vergeetProjectIcoon(pad || null) } catch { /* main is al weg */ }
}
...
// Zelfde truc als Knoppenrij.verplaatsVolgorde, maar dan op een eigen
// deellijst (bijvoorbeeld alleen de programma-knoppen) in plaats van op wat
// idsInBeeld() zou teruggeven. Nodig omdat de instellingen-secties "Knoppen"
// en "Programma's" nu apart getekend worden terwijl ze onder de motorkap nog
// dezelfde cmdVolgorde['run'] delen -- slepen in de ene lijst mag de volgorde
// van knoppen in de andere lijst niet aanraken.
function verplaatsCmdSubset(bron, sectie, subsetIds, van, naar) {
  if (naar < 0 || naar >= subsetIds.length) return false
  const volledig = Knoppenrij.volgorde(projectRij(bron, sectie))
  const inBeeld = [...subsetIds]
  if (!Knoppenrij.verschuif(inBeeld, van, naar)) return false
  const staatInBeeld = new Set(subsetIds)
  let zi = 0
...
function renderSidebar() {
  ...
  projects.forEach((p, n) => {
    ...
    div.innerHTML = `
      <div class="proj-icon">${projIcoonInhoud(p)}</div>
      <div class="proj-info">
  ...
  const chromeHtml = `
    <div class="proj-header" data-krap>
      <div class="proj-header-left" data-krap>
        <span class="proj-header-icon">${projIcoonInhoud(p)}</span>
...
function normaliseerProjectTab(t) {
  return t === 'browser' || t === 'editor' ? t : 'output'
}
...
function nieuwSlot(v) {
  if (v === 'project') {
    return { view: 'project', projectId: activeId, tab: normaliseerProjectTab(termTab) }
  }
  return { view: v }
}
  ...
  if (gemengd) {
    ...
    werkSplit.slots.forEach((s) => {
      ...
      if (s.view === 'project') {
        ...
        termTab = normaliseerProjectTab(s.tab)
      }
    ...
    if (focus) {
      ...
      if (focus.view === 'project') {
        ...
        termTab = normaliseerProjectTab(focus.tab)
      }
  ...
  if (richting === 'right' || richting === 'bottom') {
    ...
    if (houden) {
      ...
      if (view === 'project') {
        ...
        termTab = normaliseerProjectTab(houden.tab || 'output')
      }
    }
  }
...
function sluitSplitAanKant(kant) {
  ...
  if (houden) {
    view = houden.view || 'project'
    if (view === 'project') {
      const tab = normaliseerProjectTab(houden.tab)
      houden.tab = tab
      activeId = houden.projectId
      termTab = tab
      if (houden.projectId) {
...
function pasTermSchermAan() {
  if (splitGemengd() && werkSplit.slots) {
    document.querySelectorAll('#werk .terminal-wrap').forEach(w => {
      ...
      const paneEd = w.querySelector('.term-pane[data-pane="editor"]')
      if (paneOut) paneOut.hidden = tab !== 'output'
      if (paneBr) paneBr.hidden = tab !== 'browser'
      if (paneEd) paneEd.hidden = tab !== 'editor'
      ...
      if (term) term.hidden = tab !== 'output' || !!sessie
      if (br) br.hidden = tab !== 'browser'
      if (pty) pty.hidden = tab !== 'output' || !sessie
      if (inMain) {
        const editorAan = tab === 'editor'
        const outputZichtbaar = tab === 'output'
        const ptyZichtbaar = !!sessie && outputZichtbaar
  ...
  if (twee && werkSplit.slots) {
    ...
    br.hidden = !splitTabs.includes('browser')
    if (brAndere) brAndere.hidden = true
    if (pty) pty.hidden = !splitTabs.includes('output') || !sessieLeeft
  } else {
  ...
  const projRows = projects.map(p => {
    ...
    return `
      <div class="settings-proj-item" data-proj-id="${p.id}">
        <div class="settings-proj-icon">${projIcoonInhoud(p)}</div>
...
function zetInstelSectie(sleutel, open) {
  ...
  if (!kop) return
  kop.classList.toggle('dicht', !open)
  const pijl = kop.querySelector('i')
...
// Een knop is een "programma-knop" (editor of AI-dienst) als zijn id zo
// begint. Alles daarbuiten (git, flutter/tools, eigen commando's) is een
// gewone knop en hoort in de "Knoppen"-sectie, niet in "Programma's".
function isProgKnopId(id) {
  return id.startsWith('editor:') || id.startsWith('ai:')
}

function renderCmdVisibilitySection() {
  const container = document.getElementById('cmdvis-section')
  const progContainer = document.getElementById('progvis-section')
  if (!container) return
  ...
  const groupBlok = (title, sleutel, rows, toggleKey, legeTekst, dimKey) => {
    ...
    const lijst = rows.length
      ...
      : `<p class="cmdvis-leeg">${esc(legeTekst)}</p>`
    return `
  ...
  containers.forEach(doel => {
    ...
    doel.querySelectorAll('[data-cmdvis-id]').forEach(chk => {
      chk.onchange = () => { pendingCmdVisibility[chk.dataset.cmdvisId] = chk.checked }
    })
    doel.querySelectorAll('[data-del-custom]').forEach(btn => {
      btn.onclick = (ev) => {
  ...
  sleepBlokken.forEach(({ sleutel, groep, subset }) => {
    if (!groep) return
    // Dit venster toont elke knop, ook de uitgevinkte. En het project bestaat
    // hier nog niet: modalProjectCtx() geeft elke keer een vers hulpobject, dus
    // de nieuwe volgorde moet er zelf uit gehaald en in pendingCmdVolgorde gezet
    // worden. Zonder dat verdwijnt elke sleep.
    const verplaatsHier = (van, naar) => {
      const ctx = modalProjectCtx()
      const ok = subset
        ? verplaatsCmdSubset(ctx, 'run', subset, van, naar)
        : verplaatsCmdVolgorde(ctx, 'run', van, naar, 'alles', pendingCmdVisibility)
      if (!ok) return
      pendingCmdVolgorde = ctx.cmdVolgorde
      renderCmdVisibilitySection()
    }

    if (cmdvisSorteerModus === sleutel) {
    ...
    groep.querySelectorAll('.cmdvis-row').forEach(row => {
      ...
      row.onmousedown = (e) => {
        ...
        if (cmdvisSorteerModus === sleutel) return
        timer = setTimeout(() => {
          cmdvisSorteerModus = sleutel
          showToast(I18N.t('project.cmdSortToast'))
          renderCmdVisibilitySection()
        }, LANG_DRUKKEN_MS)
      }
